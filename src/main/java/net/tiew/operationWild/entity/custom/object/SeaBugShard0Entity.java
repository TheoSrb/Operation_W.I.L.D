...
public class SeaBugShard0Entity extends SeabugShard {
    ...
    private static final EntityDataAccessor<Integer> DATA_INITIAL_VARIANT = SynchedEntityData.defineId(SeaBugShard0Entity.class, EntityDataSerializers.INT);

    private static final EntityDataAccessor<Float> YROT = SynchedEntityData.defineId(SeaBugShard0Entity.class, EntityDataSerializers.FLOAT);
    ...
    public SeaBugShard0Entity(EntityType<? extends TamableAnimal> entityType, Level level, float scale, int maxSleepBar, int sleepBarDownSpeed) {
    ...
    public InteractionResult mobInteract(Player player, InteractionHand hand) {
        if (!this.level().isClientSide() && hand == InteractionHand.MAIN_HAND && player.getItemInHand(hand).isEmpty()) {
            ItemStack itemToGive = new ItemStack(OWItems.SEABUG_COCKPIT.get(), 1);
    ...
    public SpawnGroupData finalizeSpawn(ServerLevelAccessor levelAccessor, DifficultyInstance difficultyInstance, MobSpawnType mobSpawnType, @Nullable SpawnGroupData spawnGroupData) {
        this.setVariant(SeaBugShardVariant.DEFAULT_SHARD_0);
