...
public class KodiakSitGoal extends Goal {
    ...
    public boolean canUse() {
        return kodiak.getRandom().nextInt((int) (((kodiak.isDirty()) ? 300 : 500) / sitFrequencyMultiplier)) == 0 && !kodiak.isTame() &&
                ...
                !kodiak.isNapping() &&
                !kodiak.isSearchingInsideChest && !kodiak.isRolling() && !kodiak.isCatchingSalmon() && !kodiak.isTame();
    }

    @Override
    public boolean canContinueToUse() {
        if (kodiak.isSearchingInsideChest) return false;
        return kodiak.isSitting() && kodiak.getTarget() == null && !kodiak.isCatchingSalmon() && !kodiak.isTame();
    }
}