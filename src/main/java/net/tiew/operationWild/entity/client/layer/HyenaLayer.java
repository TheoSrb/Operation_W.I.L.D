...
public class HyenaLayer extends RenderLayer<HyenaEntity, HyenaModel<HyenaEntity>> {
    ...
    private static final ResourceLocation RESURRECTION_GLOWING_TEXTURE = ResourceLocation.fromNamespaceAndPath(OperationWild.MOD_ID, "textures/entity/hyena/skins/hyena_skin_gold_glowing.png");
    private static final ResourceLocation BLOODY_STAGE_0_TEXTURE = ResourceLocation.fromNamespaceAndPath(OperationWild.MOD_ID, "textures/entity/hyena/hyena_bloody_stage_0.png");
    private static final ResourceLocation BLOODY_STAGE_1_TEXTURE = ResourceLocation.fromNamespaceAndPath(OperationWild.MOD_ID, "textures/entity/hyena/hyena_bloody_stage_1.png");
    private static final ResourceLocation BLOODY_STAGE_2_TEXTURE = ResourceLocation.fromNamespaceAndPath(OperationWild.MOD_ID, "textures/entity/hyena/hyena_bloody_stage_2.png");
    private static final ResourceLocation SADDLE_TEXTURE = ResourceLocation.fromNamespaceAndPath(OperationWild.MOD_ID, "textures/entity/hyena/hyena_saddle.png");

    public HyenaLayer(HyenaRenderer hyenaRenderer) {
        super(hyenaRenderer);
    }

    @Override
    public void render(PoseStack poseStack, MultiBufferSource multiBufferSource, int packedLight, HyenaEntity hyena, float v, float v1, float v2, float v3, float v4, float v5) {
        double hyenaHealthTier = hyena.getMaxHealth() / 4;
        if (hyena.isInResurrection()) {
            float opacity = (float) (0.75 * (1 - hyena.getResurrectionPercentage() / 100.0f));
        ...
        if (hyena.isSaddled()) renderOverlay(poseStack, multiBufferSource, SADDLE_TEXTURE, false, packedLight);
        if (hyena.getHealth() < hyenaHealthTier) renderOverlay(poseStack, multiBufferSource, BLOODY_STAGE_2_TEXTURE, false, packedLight);
        else if (hyena.getHealth() < (hyenaHealthTier * 2)) renderOverlay(poseStack, multiBufferSource, BLOODY_STAGE_1_TEXTURE, false, packedLight);
        else if (hyena.getHealth() < (hyenaHealthTier * 3)) renderOverlay(poseStack, multiBufferSource, BLOODY_STAGE_0_TEXTURE, false, packedLight);
    }
