...
public class WoodenStingerRenderer extends EntityRenderer<WoodenStinger> {
    ...
    public WoodenStingerRenderer(EntityRendererProvider.Context context) {
        super(context);
        this.model = new WoodenStingerModel(context.bakeLayer(WoodenStingerModel.LAYER_LOCATION));
    }

    @Override
    public void render(WoodenStinger pEntity, float pEntityYaw, float pPartialTicks, PoseStack pPoseStack, MultiBufferSource pBuffer, int pPackedLight) {
        pPoseStack.pushPose();
    ...
    @Override
    public ResourceLocation getTextureLocation(WoodenStinger woodenStinger) {
        return ResourceLocation.fromNamespaceAndPath(OperationWild.MOD_ID, "textures/entity/wooden_stinger/wooden_stinger.png");
    }
