...
/*public class AdventurerManuscriptScreen extends Screen {
    ...
    public AdventurerManuscript cachedBookEntity;
    ...
    public final int PAGE_WIDTH = 142;

    public static final int[] ACTUAL_RIGHT_PAGE = {0, 0};
    public static final int[] ACTUAL_LEFT_PAGE = {0, 840};

    public static final int[] NEXT_RIGHT_PAGE = {360, 0};
    public static final int[] NEXT_LEFT_PAGE = {144, 0};

    public static final int[] PREVIOUS_RIGHT_PAGE = {144, 840};
    public static final int[] PREVIOUS_LEFT_PAGE = {736, 0};

    public final int X_OFFSET = 15;
    public final int Y_OFFSET = 15;
    ...
    private int nextPageTimerBeforeClear;


    public static HashMap<EntityType<? extends OWEntity>, Integer> ENTITIES = new HashMap<>();

    static {
        ENTITIES.put(OWEntityRegistry.BOA.get(), 3);
        ENTITIES.put(OWEntityRegistry.PEACOCK.get(), 3);
        ENTITIES.put(OWEntityRegistry.TIGER.get(), 4);
    }


    public AdventurerManuscriptScreen() {
        super(Component.translatable("tooltip.notebook"));
    }
    ...
    protected void init() {
        ...
        if (this.cachedBookEntity == null) {
            this.cachedBookEntity = new AdventurerManuscript(OWEntityRegistry.ADVENTURER_MANUSCRIPT.get(), Minecraft.getInstance().level);
            this.cachedBookEntity.tickCount = 1;
        }
        ...
        this.maxPages = getAllAnimalPages();
    }
    ...
    private int getChapterIndexPageFor(EntityType<? extends OWEntity> entityType) {
        int currentPage = 1;
    ...
    private int getAllAnimalPages() {
        int pages = 0;
        for (Integer $$0 : ENTITIES.values()) {
            pages += $$0;
        }
        return pages;
    }

    public EntityType<? extends OWEntity> getAnimalForPageIndex(int chapterStartPage) {
        int currentPage = 1;
    ...
    public void tick() {
        ...
        if (nextPageTimerBeforeClear > 0 && nextPageTimerBeforeClear < 8) {
            nextPageTimerBeforeClear++;
        }
    }
    ...
    public void render(GuiGraphics guiGraphics, int mouseX, int mouseY, float partialTick) {
        ...
        renderBook(guiGraphics, screenCenterX - modelRadius, (screenCenterY - modelRadius), screenCenterX + modelRadius, (int)(screenCenterY + modelRadius), 130, partialTick);

        guiGraphics.pose().popPose();
    }

    private void drawAllChapterPages(ResourceLocation baseTexture, int currentPageIndex) {
        List<OWChapter.TextLayer> allTextLayers = new ArrayList<>();
        List<OWChapter.ImageLayer> allImageLayers = new ArrayList<>();

        if (currentPageIndex > 1) {
            addPageLayers(allTextLayers, allImageLayers, currentPageIndex, PREVIOUS_LEFT_PAGE);
        }

        addPageLayers(allTextLayers, allImageLayers, currentPageIndex, ACTUAL_LEFT_PAGE);

        if (currentPageIndex < maxPages) {
            addPageLayers(allTextLayers, allImageLayers, currentPageIndex, NEXT_LEFT_PAGE);
        }

        OWChapter.TextLayer[] textArray = allTextLayers.toArray(new OWChapter.TextLayer[0]);
        OWChapter.ImageLayer[] imageArray = allImageLayers.toArray(new OWChapter.ImageLayer[0]);

        OWChapter.writeMultiLayerContentOnModelTexture(textArray, imageArray, baseTexture, false, 80);
    }

    private void addPageLayers(List<OWChapter.TextLayer> textLayers, List<OWChapter.ImageLayer> imageLayers,
                               int pageIndex, int[] pagesCoords) {
        EntityType<? extends OWEntity> entityType = getAnimalForPageIndex(pageIndex);
        if (entityType != null) {
            String displayText = entityType.getDescription().getString();

            OWEntity entity = entityType.create(Minecraft.getInstance().level);
            double tamingExp = entity != null ? entity.getTamingExperience() : 0;
            String tamingExpString = String.valueOf(tamingExp);

            int centerX = pagesCoords[0] + (PAGE_WIDTH / 2) - (this.font.width(displayText) / 2) - 5;
            int baseY = pagesCoords[1] + Y_OFFSET;

            textLayers.add(new OWChapter.TextLayer(displayText, centerX, baseY + 2, 2.0f, 0.6f, 0xc8be95));
            textLayers.add(new OWChapter.TextLayer(displayText, centerX - 2, baseY, 2.0f, 0.8f, 0x92855f));
            textLayers.add(new OWChapter.TextLayer(tamingExpString, pagesCoords[0] + (PAGE_WIDTH / 2) + 15, baseY + 25, 1.0f, 0.8f, 0x92855f));
            ...
            ResourceLocation entityChapterTexture = ResourceLocation.fromNamespaceAndPath(OperationWild.MOD_ID, "textures/gui/adventurer_manuscript/" + entityType.toString().split("entity.ow.")[1] + "_chapter.png");

            imageLayers.add(new OWChapter.ImageLayer(entityTexture, pagesCoords[0] + (PAGE_WIDTH / 2) - 2, baseY + 20, 1.35f, 0.8f, 110, 12, 11, 11));
            imageLayers.add(new OWChapter.ImageLayer(entityChapterTexture, pagesCoords[0] + (PAGE_WIDTH / 2) - (97 / 2) + 5, baseY + 37, 1.0f, 0.5f, 762, 885, 97, 114));
        }
    }

    private void renderBook(GuiGraphics guiGraphics, int x1, int y1, int x2, int y2, float scale, float partialTick) {
        float centerX = (float) (x1 + x2) / 2.0F;
        float centerY = (float) (y1 + y2) / 2.0F;
        ...
        drawAllChapterPages(baseTexture, pageIndex);
        ...
        VertexConsumer vertexConsumer = guiGraphics.bufferSource().getBuffer(RenderType.entityCutoutNoCull(baseTexture));
